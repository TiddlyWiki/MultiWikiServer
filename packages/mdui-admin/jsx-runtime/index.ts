...
import { MaybeArray, MaybeArrayDeep, Ref, is, truthy } from "./jsx-utils";
import type * as htmljsx from 'html-jsx'
import type { MDUI_Elements } from "./jsx-mdui-types";
import type { JSXElement } from "../client/utils/JSXElement";
...
declare module 'html-jsx' {
  interface DOMAttributes<T> {
    ref?: Ref<T extends Element ? T : never>;
...
declare global {
  export namespace JSX {
    ...
    export interface ElementChildrenAttribute { children: MaybeArrayDeep<JSX.Node>; }
    export type LibraryManagedAttributes<C, P> = P & {} & ToAttr<htmljsx.HTMLAttributes<C>>;
    ...
    export interface HTMLAttributes<T> extends
      Omit<htmljsx.HTMLAttributes<T>, keyof IntrinsicAttributes>,
      IntrinsicAttributes { }
    ...
    export interface IntrinsicElements extends OurElements { }
    export interface IntrinsicElements extends MDUI_Elements { }
    export interface IntrinsicElements extends html_jsx_Elements { }
    export interface IntrinsicElements extends IconElements { }
    export interface IntrinsicElements extends ElementsModified<CustomElements, any> { }
    type IgnoredProperties = "children" | "key" | "ref" | "xmlns";
    type html_jsx_Elements = ElementsModified<htmljsx.IntrinsicElements, any>
    type OurElements = ElementsModified<HTMLElementTagNameMap, JSXElement<{}>>;

    type ElementsModified<T, B> = {
      [K in keyof T as T[K] extends B ? K : never]: (
        AddWatched<{
          [P in keyof T[K]as P extends IgnoredProperties ? never : P]?:
          P extends "style" | "class" | "className" ? string :
          string extends T[K][P] ? string | number :
          T[K][P];
        }>
      ) & (
        T[K] extends JSXElement<infer E> ? keyof E extends never ? {} : {
          [P in string & keyof E]: { [P2 in `on${P}`]?: htmljsx.EventHandler<T[K], E[P]> }
        }[string & keyof E] : {}
      ) & IntrinsicAttributes & {
        [K in `webjsx-attr-${string}`]?: string
      } & {
    ...
    type ToAttr<T> = {
      [P in `webjsx-attr-${string & keyof T}` as P extends `webjsx-attr-on${string}` ? never : P]?: string;
    } & {
...
export const OwnKeySymbol: unique symbol = Symbol("__own_key__");
export const DOMElementSymbol: unique symbol = Symbol("__dom_element__");
...
export function updateElement(
  ...
  if (jsx.props) {
    if (typeof jsx.type === "function") {
      ...
      updateAttributes(jsx.props, el, (key) => ["class", "style"].includes(key) || key.startsWith("webjsx-"));
    } else {
  ...
  for (let [key, value] of Object.entries(props)) {
    ...
    if (key.startsWith("webjsx-attr-")) key = key.slice("webjsx-attr-".length);
      ...
      observerCallbacks.get(el)!.push(() => {
        ...
        ) descriptor.set?.(el.getAttribute(key));
      });
    }
...
function replaceChildren(el: ParentNode, newChildNodes: (ChildNode | string)[]) {
  ...
  for (let i = 0, j = 0; i < newChildNodes.length; i++) {
    ...
    if (newChildNode === curChildNode
      ...
      && newChildNode.textContent === curChildNode.textContent
    ) { j++; continue; }
    if (curChildNode) {
...
function isJSXDOMElement(newChildDef: JSX.Node): newChildDef is DOMElement {
  if (!newChildDef) return false;
