...
export class RecipeResolver {
  ...
  constructor(
    ...
    for (const rb of recipe.recipe_bags) {
      if (rb.is_writable) {
        const prefix = (rb.info as { prefix?: string } | null)?.prefix ?? "";
        writablePrefixBags[prefix] = rb;
      }
    }
  ...
  async serveIndexFile(
    ...
    const plugins = [...new Set([
      "$:/core",
      ...requiredPlugins,
      ...this.recipe.plugins.map((plugin) => plugin.resolved_version),
    ]).values()];
...
export function createSampleWiki(prisma: PrismaEngineClient) {
  return prisma.$transaction(async (tx) => {
    const existing = await tx.recipe.findMany({ select: { id: true } });
    if (existing.length) {
      existing.map(e => { console.log("wiki", e.id) });
      return;
    }
    ...
    await tx.recipeBag.create({
      data: {
        recipe: { connect: { slug: recipe.slug } },
