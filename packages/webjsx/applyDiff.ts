...
function canUpdateVNodes(
  ...
  if (isNonBooleanPrimitive(newVNode) && isNonBooleanPrimitive(oldVNode)) {
    ...
    if (isVElement(oldVNode) && isVElement(newVNode)) {
      ...
      return (oldVNode.tagName === newVNode.tagName && oldKey === newKey);
    } else {
      return false;
    }
  }
  ...
  for (const change of changes) {
    if (change.type === "create") {
      ...
      if (isVElement(newVNode)) {
        ...
        const isPropsAttribute = typeof newVNode.type === "function";
        updateAttributes(node as Element, newProps, oldProps, isPropsAttribute);
