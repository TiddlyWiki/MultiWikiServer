...
function updatePropOrAttr(el: Element, key: string, value: unknown): void {
  if (el instanceof HTMLElement) {
    if (key.startsWith("webjsx-attr-")) {
      const attrName = key.slice("webjsx-attr-".length);
...
function updateAttributesCore(
  ...
  isPropsAttribute: boolean
): void {
  if (isPropsAttribute) {
    (el as WebJSXManagedElement).props = {
      ...newProps,
      key: undefined,
      ref: undefined,
      children: undefined,
...
export function setAttributes(
  ...
  if (definesRenderSuspension(el)) {
    withRenderSuspension(el, () => {
      updateAttributesCore(el, props, {}, isPropsAttribute);
    });
  } else {
    updateAttributesCore(el, props, {}, isPropsAttribute);
  }
...
export function updateAttributes(
  ...
  if (definesRenderSuspension(el)) {
    withRenderSuspension(el, () => {
      updateAttributesCore(el, newProps, oldProps, isPropsAttribute);
    });
  } else {
    updateAttributesCore(el, newProps, oldProps, isPropsAttribute);
  }
