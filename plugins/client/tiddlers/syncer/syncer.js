...
class Syncer {
	constructor(options) {
		...
		this.titlesToBeLoaded = Object.create(null); // Hashmap of titles of tiddlers that need loading from the server
		...
		if($tw.browser && !this.disableUI) {
			...
			$tw.rootWidget.addEventListener("tm-login", function(event) {
				var username = event && event.paramObject && event.paramObject.username, password = event && event.paramObject && event.paramObject.password;
	...
	*/
	readTiddlerInfo() {
		...
		this.tiddlerInfo = {};
		// Record information for known tiddlers
		var self = this, tiddlers = this.getSyncedTiddlers();
	...
	isDirty() {
		...
		function checkIsDirty() {
			...
			for(var index = 0; index < titles.length; index++) {
				var title = titles[index], tiddlerInfo = self.tiddlerInfo[title];
	...
	*/
	async processTaskQueue() {
		...
		if((!this.syncadaptor.isReady || this.syncadaptor.isReady()) && this.numTasksInProgress === 0) {
			...
			var tasks = this.chooseNextTask();
			console.log("Syncer: chooseNextTask returned:", tasks);
			let taskRuns = (() => {
				if(tasks.savePending.length > 0) {
					if(this.syncadaptor.saveTiddlers) {
						return [new SaveTiddlersTask(this, tasks.savePending)];
					} else if(this.syncadaptor.saveTiddler) {
						return tasks.savePending.map(e => new SaveTiddlerTask(this, e));
					}
				}
				if(tasks.syncServer) {
					return [new SyncFromServerTask(this)];
				}
				if(tasks.deletePending.length > 0) {
					if(this.syncadaptor.deleteTiddlers) {
						return [new DeleteTiddlersTask(this, tasks.deletePending)];
					} else if(this.syncadaptor.deleteTiddler) {
						return tasks.deletePending.map(e => new DeleteTiddlerTask(this, e));
					}
				}
				if(tasks.loadPending.length > 0) {
					this.titlesToBeLoaded = Object.create(null);
					if(this.syncadaptor.loadTiddlers) {
						return [new LoadTiddlersTask(this, tasks.loadPending)];
					} else if(this.syncadaptor.loadTiddler) {
						return tasks.loadPending.map(e => new LoadTiddlerTask(this, e));
					}
				}
			})();
			console.log("Syncer: taskRuns:", taskRuns);
			if(taskRuns?.length) {
				let haserr = false;
				self.numTasksInProgress += taskRuns.length;
				await Promise.all(taskRuns.map(async (task) => {
					self.logger.log("Processing task:", task.toString());
					const err = await new Promise(r => task.run(r));
					if(err) {
				...
				if(haserr) {
					// If there was an error, trigger a retry after the errorRetryInterval
	...
	chooseNextTask() {
		...
		for(var index = 0; index < titles.length; index++) {
			...
			if(tiddler) {
				...
				var hasChanged = !tiddlerInfo || this.wiki.getChangeCount(title) > tiddlerInfo.changeCount, isReadyToSave = !tiddlerInfo || !tiddlerInfo.timestampLastSaved || tiddlerInfo.timestampLastSaved < thresholdLastSaved;
				if(hasChanged) {
					if(isReadyToSave) {
		...
		return { havePending, savePending, loadPending, deletePending, syncServer };
	}
}
...
class SaveTiddlerTask {
	constructor(syncer, title) {
		this.syncer = syncer;
		this.title = title;
		this.type = "save";
	}
...
class SaveTiddlersTask {
	constructor(syncer, titles) {
		this.syncer = syncer;
		this.titles = titles;
		this.type = "save";
	}
	toString() {
		return "SAVE " + this.titles.join(", ");
	}
	run(callback) {
		var self = this,
			changeCount = this.titles.map(title => this.syncer.wiki.getChangeCount(title)),
			tiddlers = this.titles.map(title => this.syncer.wiki.tiddlerExists(title) && this.syncer.wiki.getTiddler(title)).filter(e => e);
		this.syncer.logger.log("Dispatching 'save' task:", this.title);
		if(tiddlers.length) {
			this.syncer.syncadaptor.saveTiddlers({
				syncer: this.syncer,
				tiddlers,
				tiddlerInfo: this.titles.map(title => self.syncer.tiddlerInfo[title]),
				callback: function(err, title, adaptorInfo, revision) {
					// If there's an error, exit without changing any internal state
					if(err) {
						return callback(err);
					}
