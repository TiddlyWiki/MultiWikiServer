...
import { NormalQueryResult, QueryResult, SQLiteValue } from 'node-sqlite3-wasm';

type Row = NormalQueryResult;
...
export function getColumnTypes(declaredTypes: string[], rows: SQLiteValue[][]): ColumnType[] {
  ...
  columnLoop: for (const columnIndex of emptyIndices) {
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
      ...
      if(candidateValue === undefined) 
        console.log(rows[rowIndex], columnIndex, candidateValue)
